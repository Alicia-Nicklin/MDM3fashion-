...
Outputs (gam_output/):
    ...
    gam2_trimmed_results.csv
    gam2_trimmed_all_specialists_results.csv
"""
...
def rollout(gam, trend_df):
    ...
    for i in range(seed_end, last_pos + 1):
        ...
        X    = np.array([[grp["t_rel"].iloc[i], grp["month_sin"].iloc[i],
...
def plot_specialist_on_all9(spec_cat, results_by_trend, test_df):
    ...
    out = f"gam_output/fig_{spec_cat.lower()}_specialist_all9.png"
...
def plot_heatmap(all_results_df):
    ...
    plt.savefig("gam_output/fig_gam2_heatmap.png", dpi=140, bbox_inches="tight")
    plt.close()
...
def plot_comparison_with_gam1(gam2_results):
    ...
    plt.tight_layout()
    plt.savefig("gam_output/fig_gam1_vs_gam2_trimmed.png", dpi=140, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    all_results.to_csv("gam_output/gam2_trimmed_all_specialists_results.csv", index=False)
    gam2_results.to_csv("gam_output/gam2_trimmed_results.csv", index=False)
